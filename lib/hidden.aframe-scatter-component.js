  ...
  update: function () {
    ...
    if (data.csv) {
      ...
      var plotData = function (data) {
        ...
        var xExtent = d3.extent(data, function (d) { return d.compensation; });
        var xScale = d3.scale.linear()
                       .domain(xExtent)
                       .range([xRange[0], xRange[1]])
                       .clamp('true');

        var yExtent = d3.extent(data, function (d) { return d.willWork; });
        var yScale = d3.scale.linear()
                       .domain(yExtent)
                       .range([yRange[0], yRange[1]]);

        var zExtent = d3.extent(data, function (d) { return d.totalFI; });
        ...
        originPoint.selectAll('a-sphere')
                   ...
                   .attr('position', function (d) {
                     return xScale(d.compensation/2) + ' ' + yScale(d.willWork/2) + ' ' + zScale(d.totalFI/2);
                   })
